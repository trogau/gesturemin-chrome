...
function initialize (x, y) {
  ...
  // convert screen coordinates to CSS viewport coordinates
  const viewportScreenX = window.screenX + Math.round((window.outerWidth - window.innerWidth) / 2);
  const viewportScreenY = window.screenY + (window.outerHeight - window.innerHeight);
  lastPoint.x = Math.round(x - viewportScreenX);
  lastPoint.y = Math.round(y - viewportScreenY);
}
...
function updateGestureTrace (points) {
  ...
  for (let point of points) {
    // convert screen coordinates to CSS viewport coordinates
    point.x = Math.round(point.x - viewportScreenX);
    point.y = Math.round(point.y - viewportScreenY);
